...
def tool_source(marker: str, continuation_value: str) -> str:
    return f'''
class Tools:
    def native_phase_step_one(self, marker: str) -> dict:
        """Start the native phase ordering fixture and return a continuation value.

        :param marker: The exact public E2E marker from the user request.
        """
        if marker != {marker!r}:
            return {{"status": "error", "reason": "marker mismatch"}}
        return {{"status": "ok", "marker": {marker!r}, "continuation_value": {continuation_value!r}, "step": 1}}

    def native_phase_step_two(self, continuation_value: str) -> dict:
        """Complete the native phase ordering fixture with the value returned by step one.

        :param continuation_value: The exact continuation value returned by native_phase_step_one.
        """
        if continuation_value != {continuation_value!r}:
            return {{"status": "error", "reason": "continuation value mismatch", "step": 2}}
...
def bifrost_logs(limit: int = 5) -> list[dict[str, Any]]:
    ...
    return [entry for entry in listing.get("logs") or [] if isinstance(entry, dict)]


def bifrost_log_ids(limit: int = 5) -> list[str]:
    return [
        entry["id"]
        for entry in bifrost_logs(limit)
...
def validate_bifrost_order(marker: str, before_ids: set[str]) -> dict[str, Any]:
    ...
    while time.monotonic() < deadline and selected is None:
        after_entries = bifrost_logs()
        new_entries = [
            entry
            for entry in after_entries
            if isinstance(entry.get("id"), str) and entry["id"] not in before_ids
        ]
        new_ids = [entry["id"] for entry in new_entries]
        for entry in new_entries:
            log_id = entry["id"]
            if (
                entry.get("object") != "responses_stream"
                or entry.get("provider") != BIFROST_PROVIDER
                or entry.get("model") != BIFROST_MODEL
            ):
                continue
            if len(inspected) >= MAX_BIFROST_DETAILS:
                continue
...
def main() -> int:
    ...
    continuation_value = f"continuation-{uuid.uuid4().hex}"
    ...
    try:
        ...
        tool = request_json(
            ...
            "/api/v1/tools/create",
            {
                ...
                "content": tool_source(marker, continuation_value),
                "meta": {
        ...
        prompt = (
            ...
            "Then, only after reading its result, call native_phase_step_two with the continuation_value it returned. "
            "Do not guess or reuse a continuation value before step one returns. "
        ...
        summary["elapsed_seconds"] = round(time.monotonic() - started, 3)
        safe_summary = redact_secret(summary, continuation_value)
