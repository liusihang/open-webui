...
- 2026-07-27 G6: scope review confirmed no backend/admin/Docker/remote/live changes, no frontend Administrator Prompt field, no profile model selection, and no reasoning-depth mutation. Model-change resolution retains only permitted selections and explicitly removes Code Interpreter when Terminal remains selected.
- 2026-07-27 G7: committed only the listed Phase G frontend/tests/handoff files with `feat(chat): apply mode-specific capability defaults`; no push performed.
