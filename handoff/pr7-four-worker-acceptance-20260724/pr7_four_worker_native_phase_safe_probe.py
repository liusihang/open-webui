...
class Tools:
    def native_phase_step_one(self, marker: str) -> dict:
        ...
        return {{"status": "ok", "marker": {marker!r}, "continuation": {continuation_value!r}, "step": 1}}

    def native_phase_step_two(self, continuation: str) -> dict:
        """Complete the native phase ordering fixture with the value returned by step one.

        :param continuation: The exact continuation value returned by native_phase_step_one.
        """
        if continuation != {continuation_value!r}:
...
def main() -> int:
    ...
    continuation_value = "step-one-result-acceptance"
    ...
    try:
        ...
        prompt = (
            ...
            "Then, only after reading its result, call native_phase_step_two with the continuation it returned. "
            "Do not guess or reuse a continuation before step one returns. "
