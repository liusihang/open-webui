...
export const getDeepResearchConfig = async (token: string) => {
	let error = null;

	const res = await fetch(`${WEBUI_API_BASE_URL}/configs/deep_research`, {
		method: 'GET',
		headers: {
			'Content-Type': 'application/json',
			Authorization: `Bearer ${token}`
...
export const setDeepResearchConfig = async (token: string, config: object) => {
	let error = null;
