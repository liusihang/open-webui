...
<script lang="ts">
	...
	export let onRegenerateLayer: (layerType: KnowledgeLayerType) => void | Promise<void> = async () => {};
				...
				<div class="rounded-lg border border-gray-100 dark:border-gray-850 p-2.5 space-y-2">
					...
					{#if card.updatedAt}
						<div
							...
							title={dayjs(card.updatedAt * 1000).format('YYYY-MM-DD HH:mm:ss')}
						>
							{$i18n.t('Updated {{time}}', {
								time: dayjs(card.updatedAt * 1000).fromNow()
							})}
						</div>
					{/if}

					<div class="text-xs text-gray-700 dark:text-gray-300 whitespace-pre-wrap line-clamp-4 leading-5">
						{#if card.content}
							{card.content}
						{:else}
							<span class="text-gray-500 dark:text-gray-400 italic">
								{$i18n.t(getLayerFallbackCopy(card.status))}
							</span>
						{/if}
					</div>
				</div>
