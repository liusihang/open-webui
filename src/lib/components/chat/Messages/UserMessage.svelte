...
>
	{#if !($settings?.chatBubble ?? true) && !(message?.meta?.internal === true && message?.meta?.type === 'timer')}
	...
	>
		...
		<div class="chat-{message.role} w-full min-w-full">
			...
			{#if edit !== true && !(message?.meta?.internal === true && message?.meta?.type === 'timer')}
				<div
