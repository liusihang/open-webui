				...
				try {
					...
					if (restoredQueue.length > 0) {
						...
						if (!hasPendingTask) {
							...
							// Has pending tasks - migrate legacy session queue into the shared queue store.
							chatRequestQueues.update((q) => ({
								...q,
								[chatIdProp]: restoredQueue
							}));
						}
					}
				} catch (e) {}
				...
				if (message.content == '' && value == '\n') {
					...
					if ($showCallOverlay) {
						const messageContentParts = getMessageContentParts(
							removeAllDetails(message.responseContent ?? message.content),
...
<div
	class="deerflow-main-shell h-screen max-h-[100dvh] transition-width duration-200 ease-in-out {$showSidebar
