...
class FakeApprovalStore:
    ...
    async def record_decision_execution(
        ...
        *,
        ...
        for event in self.events:
            if (
                event['event_type'] == 'approval.completed'
                and event['payload'].get('approval_id') == resource_id
            ):
                if event['payload'].get('decision') != decision:
                    raise AgentRunDecisionConflict('approval already has a different decision')
                return SimpleNamespace(
                    execution=None,
                    created=False,
    ...
    async def validate_approved_tool_replay(
        ...
        *,
        ...
        replay = self.approved_replays.get(execution_id)
        if replay != {
            'run_id': run_id,
            ...
            return None
        return SimpleNamespace(id=execution_id, status='activating')
...
async def test_read_only_tool_call_endpoint_bypasses_approval_and_executes_once():
    ...
    assert [event['event_type'] for event in store.events] == ['tool.completed']
...
@pytest.mark.asyncio
async def test_destructive_tool_call_records_approval_without_executing_tool():
    calls = []
    ...
    assert store.state['run-1'] == 'waiting_approval'
    assert [event['event_type'] for event in store.events] == ['approval.requested']
    assert store.events[0]['payload']['checkpoint_version'] == 7

    recorded = await coordinator.decide(
        ApprovalDecisionRequest(
            run_id='run-1',
    ...
    assert recorded['status'] == 'approval_recorded'
    assert recorded['execution_status'] == 'pending'
    assert calls == []
    assert store.state['run-1'] == 'waiting_approval'
    assert [event['event_type'] for event in store.events] == ['approval.requested']
...
@pytest.mark.asyncio
async def test_approved_decision_execution_grant_bypasses_approval_for_exact_tool_call():
    calls = []
    ...
    store = FakeApprovalStore()
    store.approved_replays['execution-1'] = {
        'run_id': 'run-1',
        'tool_call_id': 'call-1',
        'tool_id': 'tool:terminal:main:write_file',
        'arguments': {'path': '/workspace/report.txt', 'content': 'replacement'},
...
@pytest.mark.asyncio
async def test_decision_execution_grant_cannot_authorize_a_different_tool_call():
    async def write_file(path: str, content: str):
        return {'written': path}
    ...
    store.approved_replays['execution-1'] = {
        ...
        'tool_id': 'tool:terminal:main:write_file',
        'arguments': {'path': '/workspace/report.txt', 'content': 'replacement'},
        'idempotency_key': 'tool:leader:call-original:1',
    }
    authority = AgentToolAuthority(operation_store=store, registry=registry)
    coordinator = AgentApprovalCoordinator(store)

    with pytest.raises(HTTPException) as exc_info:
        await execute_agent_run_tool_call(
            request=_service_request(authority),
            run_id='run-1',
            form_data=ToolCallRequest(
                run_id='run-1',
                ...
                arguments={'path': '/workspace/report.txt', 'content': 'replacement'},
                checkpoint_version=7,
                idempotency_key='tool:leader:call-forged:1',
            ),
            idempotency_key='tool:leader:call-forged:1',
            authorization='Bearer service-secret',
            decision_execution_id='execution-1',
    ...
    assert exc_info.value.detail['code'] == 'approval_decision_conflict'
...
@pytest.mark.parametrize(
    ...
    [
        (
            {'path': '/workspace/other.txt', 'content': 'replacement'},
            'tool:leader:call-1:1',
        ),
        (
            {'path': '/workspace/report.txt', 'content': 'replacement'},
            'tool:leader:call-1:forged',
        ),
    ],
)
async def test_decision_execution_grant_rejects_changed_arguments_or_key(
    arguments,
    idempotency_key,
):
    calls = []
    ...
    with pytest.raises(HTTPException) as exc_info:
        await execute_agent_run_tool_call(
            ...
            authorization='Bearer service-secret',
            decision_execution_id='execution-1',
            approval_coordinator=AgentApprovalCoordinator(store),
        )
...
@pytest.mark.asyncio
async def test_service_tool_call_returns_approval_required_without_long_polling():
    calls = []
    ...
    decision_payload = json.loads(decision_response.body)
    assert decision_response.status_code == 202
    assert decision_payload['status'] == 'approval_recorded'
    assert decision_payload['execution_status'] == 'pending'
    assert tool_response['status'] == 'approval_required'
    assert calls == []
    assert store.state['run-1'] == 'waiting_approval'
    assert [event['event_type'] for event in store.events] == ['approval.requested']
...
async def test_destructive_tool_call_rejection_returns_normalized_rejection_result():
    ...
    assert rejected['status'] == 'approval_recorded'
    assert rejected['execution_status'] == 'pending'
    assert store.state['run-1'] == 'waiting_approval'
    assert [event['event_type'] for event in store.events] == ['approval.requested']
...
@pytest.mark.asyncio
async def test_rejected_approval_decision_records_outbox_without_direct_runtime_call():
    store = FakeApprovalStore()
    coordinator = AgentApprovalCoordinator(store)
    request = ToolCallRequest(
        run_id='run-1',
    ...
    assert response_payload['status'] == 'approval_recorded'
    assert response_payload['execution_status'] == 'pending'
...
@pytest.mark.asyncio
async def test_concurrent_approval_decision_does_not_take_over_live_operation_owner():
    store = FakeApprovalStore()
    coordinator = AgentApprovalCoordinator(store)
    request = ToolCallRequest(
        run_id='run-1',
        ...
        arguments={'path': '/workspace/report.txt', 'content': 'replacement'},
        checkpoint_version=7,
        idempotency_key='tool:leader:call-concurrent:1',
    )
    ...
    decision = ApprovalDecisionRequest(
        run_id='run-1',
        approval_id='approval:run-1:call-concurrent',
        decision='approved',
        idempotency_key='approval:run-1:call-concurrent:approved',
    )

    owner = asyncio.create_task(coordinator.decide(decision))
    contender = asyncio.create_task(
        coordinator.decide(
            decision.model_copy(update={'idempotency_key': 'approval:second-key'})
        )
    )
...
@pytest.mark.asyncio
async def test_approval_decision_recovers_in_progress_operation_from_completed_event():
    store = FakeApprovalStore()
    coordinator = AgentApprovalCoordinator(store)
    request = ToolCallRequest(
        run_id='run-1',
        ...
        arguments={'path': '/workspace/report.txt', 'content': 'replacement'},
        checkpoint_version=7,
        idempotency_key='tool:leader:call-recover:1',
    )
    await coordinator.request_tool_approval(
        request,
        {
    ...
    decision = ApprovalDecisionRequest(
        run_id='run-1',
        approval_id='approval:run-1:call-recover',
        decision='approved',
        idempotency_key='approval:run-1:call-recover:approved',
    )
    await coordinator.decide(decision)
    operation_key = next(
        key for key in store.claims if key[1] == 'approval.result'
    )
    ...
    recovered = await AgentApprovalCoordinator(store).decide(decision)

    assert recovered['status'] == 'approval_recorded'
    assert recovered['raw']['approval_id'] == decision.approval_id
