...
class KnowledgeFileLayerTable:
        ...
        *,
        ...
        with get_db_context(db) as db:
            ...
            if request is None or not getattr(
                ...
                raise RuntimeError(
                    "Layered knowledge vector retrieval requires request embedding context"
                )

            embedding = await request.app.state.EMBEDDING_FUNCTION(search_query)
            filter_payload = {"layer_type": normalized_layer}
            if knowledge_ids:
                filter_payload["knowledge_id"] = (
                    knowledge_ids[0]
                    if len(knowledge_ids) == 1
                    else {"$in": knowledge_ids}
                )
            if file_ids:
                filter_payload["file_id"] = (
                    file_ids[0] if len(file_ids) == 1 else {"$in": file_ids}
                )

            if not VECTOR_DB_CLIENT.has_collection("knowledge-layers"):
                return []
            ...
            for index, metadata in enumerate(metadatas):
                ...
                base_source = file.filename or row.title or row.file_id
                hydrated.append(
    ...
    def _set_embedding_state_for_rows(
        self,
        rows: list[KnowledgeFileLayer],
        *,
        status: str,
        error: Optional[str] = None,
    ) -> int:
        now = int(time.time())
        normalized_status = _normalize_embedding_status(status)
        for row in rows:
            row.embedding_status = normalized_status
            row.embedding_error = error
            row.embedding_updated_at = now
        return len(rows)

    def mark_embedding_indexing(
        self, row_id: str, db: Optional[Session] = None
    ) -> Optional[KnowledgeFileLayerModel]:
        with get_db_context(db) as db:
            row = db.query(KnowledgeFileLayer).filter_by(id=row_id).first()
            if not row:
                return None
            self._set_embedding_state_for_rows([row], status="indexing")
            db.commit()
            db.refresh(row)
            return KnowledgeFileLayerModel.model_validate(row)
    ...
    def mark_embedding_ready(
        ...
        with get_db_context(db) as db:
            ...
            if not row:
                return None
            self._set_embedding_state_for_rows([row], status="ready")
            db.commit()
            db.refresh(row)
            return KnowledgeFileLayerModel.model_validate(row)

    def mark_embedding_failed(
        self, row_id: str, error: str, db: Optional[Session] = None
    ) -> Optional[KnowledgeFileLayerModel]:
        with get_db_context(db) as db:
            row = db.query(KnowledgeFileLayer).filter_by(id=row_id).first()
            if not row:
                return None
            self._set_embedding_state_for_rows([row], status="failed", error=error)
            db.commit()
            db.refresh(row)
            return KnowledgeFileLayerModel.model_validate(row)
