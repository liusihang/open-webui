...
RETRIEVAL_CONFIG_KEYS = {
    ...
    'PADDLEOCR_VL_ALLOWED_REMOTE_ORIGINS': 'rag.paddleocr_vl_allowed_remote_origins',
    'PADDLEOCR_VL_DOWNLOAD_TIMEOUT': 'rag.paddleocr_vl_download_timeout',
    'PADDLEOCR_VL_MODEL': 'rag.paddleocr_vl_model',
    'PADDLEOCR_VL_OPTIONAL_PAYLOAD': 'rag.paddleocr_vl_optional_payload',
    'PADDLEOCR_VL_POLL_INTERVAL': 'rag.paddleocr_vl_poll_interval',
    'PADDLEOCR_VL_POLL_TIMEOUT': 'rag.paddleocr_vl_poll_timeout',
    'PADDLEOCR_VL_REQUEST_TIMEOUT': 'rag.paddleocr_vl_request_timeout',
    ...
    'RAG_EXTRACT_DOCUMENT_IMAGE_ASSETS': 'rag.extract_document_image_assets',
...
async def update_rag_config(request: Request, form_data: ConfigForm, user=Depends(get_admin_user)):
    ...
    config.PADDLEOCR_VL_MODEL = (
        form_data.PADDLEOCR_VL_MODEL if form_data.PADDLEOCR_VL_MODEL is not None else config.PADDLEOCR_VL_MODEL
    )
    ...
    return {
        ...
        'BYPASS_EMBEDDING_AND_RETRIEVAL': config.BYPASS_EMBEDDING_AND_RETRIEVAL,
        'RAG_FULL_CONTEXT': config.RAG_FULL_CONTEXT,
        'ENABLE_MULTIMODAL_KNOWLEDGE_EVIDENCE': config.ENABLE_MULTIMODAL_KNOWLEDGE_EVIDENCE,
        # Hybrid search settings
        'ENABLE_RAG_HYBRID_SEARCH': config.ENABLE_RAG_HYBRID_SEARCH,
        'TOP_K_RERANKER': config.TOP_K_RERANKER,
        'RELEVANCE_THRESHOLD': config.RELEVANCE_THRESHOLD,
...
def _current_chunker_config(config: RetrievalConfig) -> dict:
    return {
        'version': 1,
...
def _build_vector_metadatas(
    ...
    chunker_config = _current_chunker_config(config)
...
def save_docs_to_vector_db(
    ...
    metadatas = _build_vector_metadatas(docs, config, metadata)
