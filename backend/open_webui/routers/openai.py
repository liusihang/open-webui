...
_CLIPROXY_FILE_ID_PATTERN = re.compile(
    ...
    for msg in messages:
        ...
        for part in content:
            ...
            try:
                ...
                log.debug(f"CLIProxy image inlining skipped: read failed ({file_id}): {e}")
...
def _build_cliproxy_file_parts(
    ...
    for file_item in parent_files:
        ...
        try:
            ...
            encoded_content = base64.b64encode(raw_content).decode("utf-8")
            file_data = f"data:{content_type};base64,{encoded_content}"
...
async def generate_chat_completion(
    ...
    bypass_filter: Optional[bool] = None,
    ...
    request_bypass_filter = getattr(getattr(request, 'state', None), 'bypass_filter', False)
    if bypass_filter is None:
        bypass_filter = request_bypass_filter
    ...
    if not api_type and is_openai_new_model(payload.get('model', '')):
        api_type = 'responses'

    payload = _inline_cliproxy_image_urls_in_payload(payload, metadata, user)

    if cliproxy_api_enabled or api_type == 'responses':
