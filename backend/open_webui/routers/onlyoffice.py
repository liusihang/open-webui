...
def _require_onlyoffice_enabled(request: Request) -> str:
    if not request.app.state.config.ENABLE_ONLYOFFICE_PREVIEW:
        raise HTTPException(
    ...
    document_server_url = (
        request.app.state.config.ONLYOFFICE_DOCUMENT_SERVER_URL or ""
    ).strip()
...
def _resolve_public_base_url(request: Request) -> str:
    configured_webui_url = (request.app.state.config.WEBUI_URL or "").strip().rstrip("/")
...
def _resolve_onlyoffice_public_base_url(request: Request) -> str:
    configured_onlyoffice_base_url = (
        request.app.state.config.ONLYOFFICE_PUBLIC_BASE_URL or ""
    ).strip().rstrip("/")
...
def _parse_file_token_ttl(request: Request) -> Optional[timedelta]:
    raw = request.app.state.config.ONLYOFFICE_FILE_TOKEN_EXPIRES_IN
    try:
