...
def build_loader_from_config(request, config: dict | None = None):
    ...
    if config is None:
        runtime_config = getattr(getattr(request, 'app', None), 'state', None)
        runtime_config = getattr(runtime_config, 'config', None)
        loader_config = {
...
async def query_doc_with_hybrid_search(
    ...
    try:
        if not enable_enriched_texts:
            try:
                manifest_result = await query_manifest_hybrid_search(
                ...
                log.info(
                    'query_doc_with_hybrid_search:manifest_result '
                    + f'{manifest_result["metadatas"]} {manifest_result["distances"]}'
                )
                return manifest_result
            except HybridManifestNotReady as e:
                log.debug(
                    'Hybrid manifest not ready for collection %s; falling back to native/legacy hybrid search: %s',
                    collection_name,
                    e,
                )
            except HybridSearchFailed as e:
                log.warning(
                    'Hybrid manifest search failed for collection %s; falling back to native/legacy hybrid search: %s',
                    collection_name,
                    e,
                    exc_info=True,
                )
...
async def query_collection_with_hybrid_search(
    ...
    if not enable_enriched_texts:
        try:
            ...
            log.debug(
                'Hybrid manifest not ready for collections %s; falling back to native/legacy hybrid search: %s',
                collection_names,
                e,
            )
        except HybridSearchFailed as e:
            log.warning(
                'Hybrid manifest search failed for collections %s; falling back to native/legacy hybrid search: %s',
                collection_names,
                e,
                exc_info=True,
            )
