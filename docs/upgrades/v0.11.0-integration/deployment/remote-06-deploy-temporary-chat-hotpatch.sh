...
db_user='webui_pr7'
database='webui_pr7'
expected_current_image_id='sha256:38a2aa1b41fd1107254ca8ff36f0d1059ff4d3d0d79bf6b480a2c610520cbe6f'
expected_formal_image_id='sha256:ab6d8f1816a40750a98bdcb18e5a7bd419869c43825a66631acc7f718e6f469b'
base_image='open-webui:v011-hotfix-98ae1cd6071d'
hotpatch_image='open-webui:v011-hotfix-17cf77c906d2'
source_commit='17cf77c906d2c69d4b55e146a0ce29c6b1b0797c'
target_revision='a11c0d3f0bd0'
artifact='/tmp/openwebui-v011-hotfix-17cf77c906d2-context.tar.gz'
expected_artifact_sha256='4486f896b57f7d78a17cf6cce5fd41c221367fe7ca419a10f50b5773ae0e320f'
deployment_root='/home/aiserver/staging/openwebui-pr7-eea11194ed-test'
context_dir="${deployment_root}/hotpatch-v011-17cf77c906d2"
overlay="${deployment_root}/compose.webui-v011-hotfix-17cf77c906d2.yaml"
backup_dir="${deployment_root}/backups/pre-hotpatch-17cf77c906d2"
...
docker build \
    ...
    --file "${context_dir}/Dockerfile.hotpatch-17cf77c906d2" \
    --tag "${hotpatch_image}" \
    "${context_dir}"
...
[[ "${image_scope}" == 'v0.11-temporary-chat-id' ]]
...
services:
  open-webui-pr7:
    image: open-webui:v011-hotfix-17cf77c906d2
    environment:
...
docker exec -i "${target_container}" python - <<'PY' >"${backup_dir}/temporary-chat-runtime.txt"
from open_webui.utils.chat_id import is_saved_chat_id, is_temporary_chat_id

assert is_temporary_chat_id('temporary:browser-session') is True
assert is_saved_chat_id('temporary:browser-session') is False
assert is_temporary_chat_id('local:legacy-session') is True
assert is_saved_chat_id('channel:channel-1') is False
print('temporary_chat_id_runtime=ok')
PY
