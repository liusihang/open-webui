...
- Full focused verification passed after projector and job-level activation changes with `WEBUI_SECRET_KEY=test-secret ENABLE_DB_MIGRATIONS=false DATABASE_ENABLE_SESSION_SHARING=true uv run --frozen pytest backend/open_webui/test/util/test_evidence_projector.py -q`: `12 passed, 3 warnings`.
- `git diff --check` passed. `uv.lock` was restored after initial non-frozen `uv run` refreshed it; no `uv.lock` diff remains.
- Extended TDD for job-level embedding failures: project jobs should call projection with activation deferred and finalize only after embedding writes succeed. RED confirmed with `WEBUI_SECRET_KEY=test-secret ENABLE_DB_MIGRATIONS=false DATABASE_ENABLE_SESSION_SHARING=true uv run --frozen pytest backend/open_webui/test/util/test_evidence_projector.py::test_evidence_job_branch_uses_existing_job_state_surface -q`: failed because `activate` was still `True`.
- Implemented job-level deferred activation in `retrieval/indexing.py` using `finalize_projected_evidence_from_job_payload()` after embedding success. GREEN confirmed for `test_evidence_job_branch_uses_existing_job_state_surface` and `test_evidence_job_branch_fails_when_embeddings_are_skipped`: each `1 passed, 3 warnings`.
- After a concurrent worker advanced HEAD to `38160efb2`, reran `WEBUI_SECRET_KEY=test-secret ENABLE_DB_MIGRATIONS=false DATABASE_ENABLE_SESSION_SHARING=true uv run --frozen pytest backend/open_webui/test/util/test_evidence_projector.py -q`: `12 passed, 3 warnings`; `git diff --check` also passed.
