...
def merge_fragments(paths: list[Path], *, base_commit: str | None = None) -> dict[str, Any]:
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description='Merge W12B worker evidence fragments.')
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        document = merge_fragments(args.fragments, base_commit=args.base_commit)
        write_document(document, args.output)
    except Exception as exc:
        print(f'merge W12B evidence failed: {exc}', file=sys.stderr)
        return 2
    return 0
